...
regularizer = l2
lbdaModel = dualLambdaModel([0.0005/x for x in [84,192, 3, 5, 4, 4, 5]], 0.00001)
...
testSrc = [x for x in AdomDataSource('fold2-test.csv')]
trainSrc = [x for x in AdomDataSource('fold2-train.csv')]
sink = MetaSink(trainSink = RMSErrorSink(), testSink = RMSErrorSink(), predictSink=RMSErrorSink())
results = open('result-adom.csv', 'w')
results.write(' ITER TRAINERROR TESTERROR \n' )
